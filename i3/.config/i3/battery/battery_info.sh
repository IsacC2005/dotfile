...
then
    ...
    # 1. Quitamos el símbolo % para poder comparar el número
    LEVEL_NUM=$(echo "$BAT_LEVEL" | tr -d '%')

    # 2. "Switch" (Case en Bash) para cambiar el icono
    if [ "$IS_CHARGING" = "Charging" ]; then
        ICON="⚡"
    elif [ "$LEVEL_NUM" -ge 90 ]; then
        ICON=""  # Batería llena
    elif [ "$LEVEL_NUM" -ge 60 ]; then
        ICON=""  # Batería 3/4
    elif [ "$LEVEL_NUM" -ge 40 ]; then
        ICON=""  # Batería media
    elif [ "$LEVEL_NUM" -ge 10 ]; then
        ICON=""  # Batería baja
    else
        ICON=""  # Batería crítica
    fi

    # 3. Imprimimos el resultado para i3blocks
